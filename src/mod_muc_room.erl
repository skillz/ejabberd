	  ...
	  case check_password(ServiceAffiliation, Affiliation,
		...
		NewStateData =
		      if not IsSubscribeRequest ->
			      ...
			      ?DEBUG("(~s) Logging for joining room = ~s on host = ~s and found online room = ~p and valid room id", [Room, Host, mod_muc:find_online_room(Room, Host), mod_muc:check_create_roomid(ServerHost, Room)]),
			      case {mod_muc:find_online_room(Room, Host), mod_muc:check_create_roomid(ServerHost, Room)} of
			       {error, true} ->
			      	 ...
			      	 ?DEBUG("(~s) Logging for creating new room on serverhost = ~s from = ~s", [ServerHost, From]),
